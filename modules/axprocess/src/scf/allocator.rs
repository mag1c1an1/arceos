...
use allocator::{BaseAllocator, BuddyByteAllocator, ByteAllocator};

use super::cfg::*;
use axhal::mem::{phys_to_virt, PhysAddr, VirtAddr};
use axsync::Mutex;

const DATA_BUF_BASE: PhysAddr = PhysAddr::from(SYSCALL_DATA_BUF_PADDR);

pub struct SyscallDataBuffer {
    base_vaddr: usize,
    size: usize,
    heap: Mutex<BuddyByteAllocator>,
}

static DATA_BUFFER: SyscallDataBuffer =
    SyscallDataBuffer::new(phys_to_virt(DATA_BUF_BASE), SYSCALL_DATA_BUF_SIZE);
...
impl SyscallDataBuffer {
    ...
    const fn new(base_vaddr: VirtAddr, buf_size: usize) -> Self {
        Self {
            ...
            size: buf_size,
            heap: Mutex::new(BuddyByteAllocator::new()),
        }
    }
