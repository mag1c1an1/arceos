...
cfg_if::cfg_if! {
    ...
    if #[cfg(all(
        ...
        mod config;
    } else if #[cfg(all(any(target_arch = "x86_64",not(target_os="none")),feature = "platform-pc-x86-hv-guest"))]{
        #[rustfmt::skip]
        #[path = "config_pc_x86_hv_guest.rs"]
        mod config;
    }else if #[cfg(all(any(target_arch = "x86_64",not(target_os="none")),feature = "platform-pc-x86-hv"))]{
        #[rustfmt::skip]
        #[path = "config_pc_x86_hv.rs"]
        mod config;
    }else if #[cfg(all(
